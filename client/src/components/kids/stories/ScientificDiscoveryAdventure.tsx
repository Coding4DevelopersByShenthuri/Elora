...
import { Sparkles, FlaskConical, Gauge, Volume2, X, Zap, Play, FileText, RotateCcw, Microscope } from 'lucide-react';
import OnlineTTS, { STORY_VOICES } from '@/services/OnlineTTS';
import KidsListeningAnalytics, { type StorySession } from '@/services/KidsListeningAnalytics';
import { cn } from '@/lib/utils';
import { useAuth } from '@/contexts/AuthContext';

const QUEST_VOICE = STORY_VOICES.QuestTeen;
...
const storySteps = [
  {
    ...
    text: 'Welcome to the Research Institute! You\'re about to become a young scientist. You\'ll learn about the scientific method, experiments, observations, and how discoveries change the world. Your mission is to listen carefully and answer questions to earn THREE research badges. Are you ready to explore the world of science?',
    emoji: '🔬',
    character: 'Quest',
  ...
  },
  {
    ...
    interactive: true,
    listeningFirst: true,
    audioText: 'Scientists observe carefully to gather information',
    audioInstruction: 'Listen to what scientists do to gather information.',
    question: 'How do scientists gather information?',
    hint: 'Think about what scientists do first in their work',
    choices: [
      { text: 'Scientists guess randomly', emoji: '🎲❌', meaning: 'guessing' },
      { text: 'Scientists observe carefully to gather information', emoji: '👁️✅', meaning: 'observing' },
      { text: 'Scientists ignore everything', emoji: '🙈❌', meaning: 'ignoring' }
    ],
    revealText: 'Exactly! Scientists observe carefully to gather information. Observation is the first step in the scientific method.',
    maxReplays: 5,
    wordCount: 35,
    duration: 28
  },
  {
    ...
    interactive: true,
    listeningFirst: true,
    audioText: 'A hypothesis is an educated guess based on observations',
    audioInstruction: 'Listen to what a hypothesis is.',
    question: 'What is a hypothesis?',
    hint: 'Think about what scientists make after observing',
    choices: [
      { text: 'A hypothesis is a random thought', emoji: '🤔❌', meaning: 'random' },
      { text: 'A hypothesis is an educated guess based on observations', emoji: '💡✅', meaning: 'educated' },
      { text: 'A hypothesis is always wrong', emoji: '❌❌', meaning: 'wrong' }
    ],
    revealText: '🌟 First research badge earned! A hypothesis is an educated guess based on observations. Two more badges to go! 🌟',
    maxReplays: 5,
    wordCount: 40,
    duration: 32
  },
  {
    ...
    interactive: true,
    listeningFirst: true,
    audioText: 'Experiments test hypotheses under controlled conditions',
    audioInstruction: 'Listen to what experiments do.',
    question: 'What do experiments do?',
    hint: 'Think about how scientists test their ideas',
    choices: [
      { text: 'Experiments just make noise', emoji: '🔊❌', meaning: 'noise' },
      { text: 'Experiments test hypotheses under controlled conditions', emoji: '🧪✅', meaning: 'testing' },
      { text: 'Experiments are always dangerous', emoji: '⚠️❌', meaning: 'dangerous' }
    ],
    revealText: 'Perfect! Experiments test hypotheses under controlled conditions, helping scientists find answers.',
    maxReplays: 5,
    wordCount: 40,
    duration: 32
  },
  {
    ...
    interactive: true,
    listeningFirst: true,
    audioText: 'Data are facts and measurements collected during experiments',
    audioInstruction: 'Listen to what data are.',
    question: 'What are data?',
    hint: 'Think about what scientists collect during experiments',
    choices: [
      { text: 'Data are just opinions', emoji: '💭❌', meaning: 'opinions' },
      { text: 'Data are facts and measurements collected during experiments', emoji: '📊✅', meaning: 'facts' },
      { text: 'Data are always wrong', emoji: '❌❌', meaning: 'wrong' }
    ],
    revealText: 'Correct! Data are facts and measurements collected during experiments. They help scientists understand what happened.',
    maxReplays: 5,
    wordCount: 40,
    duration: 32
  },
  {
    ...
    interactive: true,
    listeningFirst: true,
    audioText: 'Conclusions explain what the data mean and whether the hypothesis was correct',
    audioInstruction: 'Listen to what conclusions do.',
    question: 'What do conclusions do?',
    hint: 'Think about what scientists do after collecting data',
    choices: [
      { text: 'Conclusions are just guesses', emoji: '🎯❌', meaning: 'guessing' },
      { text: 'Conclusions explain what the data mean and whether the hypothesis was correct', emoji: '📝✅', meaning: 'explaining' },
      { text: 'Conclusions are always boring', emoji: '😴❌', meaning: 'boring' }
    ],
    revealText: '🌟 Second research badge! Conclusions explain what the data mean and whether the hypothesis was correct. One more badge to go! 🌟',
    maxReplays: 5,
    wordCount: 45,
    duration: 34
  },
  {
    ...
    interactive: true,
    listeningFirst: true,
    audioText: 'Peer review means other scientists check your work for accuracy',
    audioInstruction: 'Listen to what peer review means.',
    question: 'What does peer review mean?',
    hint: 'Think about how scientists check each other\'s work',
    choices: [
      { text: 'Peer review means working alone', emoji: '🏃❌', meaning: 'alone' },
      { text: 'Peer review means other scientists check your work for accuracy', emoji: '👥✅', meaning: 'checking' },
      { text: 'Peer review means copying others', emoji: '📋❌', meaning: 'copying' }
    ],
    revealText: 'Excellent! Peer review means other scientists check your work for accuracy, ensuring quality in science.',
    maxReplays: 5,
    wordCount: 45,
    duration: 34
  },
  {
    ...
    interactive: true,
    listeningFirst: true,
    audioText: 'Scientific discoveries help us understand the world and solve problems',
    audioInstruction: 'Listen to what scientific discoveries do.',
    question: 'What do scientific discoveries do?',
    hint: 'Think about how discoveries help humanity',
    choices: [
      { text: 'Scientific discoveries cause problems', emoji: '💥❌', meaning: 'problems' },
      { text: 'Scientific discoveries help us understand the world and solve problems', emoji: '🌟✅', meaning: 'helping' },
      { text: 'Scientific discoveries are useless', emoji: '🚫❌', meaning: 'useless' }
    ],
    revealText: '🌟 Final research badge earned! Scientific discoveries help us understand the world and solve problems. You\'re now a certified Young Scientist! 🌟',
    maxReplays: 5,
    wordCount: 50,
    duration: 36
  },
  {
    id: 'debrief',
    title: '✅ Scientific Method Debrief',
    text: 'Outstanding work! You\'ve learned about observation, hypothesis, experiment, data, conclusion, peer review, and discovery. You\'re now equipped with the scientific method to explore and understand the world! Remember, science is about asking questions and finding answers!',
    emoji: '✅',
    character: 'Quest',
    bgColor: 'from-cyan-100 to-blue-100 dark:from-cyan-900 dark:to-blue-900',
    interactive: false,
  ...
  {
    id: 'celebrate',
    title: '🎉 Young Scientist Badge Earned',
    text: '',
    emoji: '🎉',
    character: 'Quest',
...
const ScientificDiscoveryAdventure = ({ onClose, onComplete }: Props) => {
  const { user } = useAuth();
  ...
  const getCelebrationText = (): string => {
    if (current.id === 'celebrate') {
      if (stars >= 3) return "Outstanding! You earned all three research badges! Your scientific potential is inspiring. Keep exploring and discovering—you're destined to be a great scientist!";
      if (stars === 2) return "Great work! You earned two research badges. Your scientific skills are growing—one more badge next time for full Young Scientist status! Keep learning!";
      if (stars === 1) return "Good progress! You earned one research badge. Keep developing your scientific skills and you'll become a Young Scientist!";
      return "Scientific journey complete. Every experiment teaches something. Keep exploring and discovering!";
    }
  ...
  useEffect(() => {
    ...
    const initSession = async () => { 
      ...
      const s = KidsListeningAnalytics.startSession(userId, 'scientific-discovery-teen', 'Scientific Discovery (Teen)'); 
      setCurrentSession(s); 
    };
    initSession();
  ...
  const playAudioWithCaptions = async (text: string) => {
    try {
      ...
      await OnlineTTS.speak(clean, QUEST_VOICE, { speed: playbackSpeed, showCaptions: false, onCaptionUpdate: () => {} });
    } catch (error) {
  ...
  const handleChoice = async (choiceObj: any) => {
    ...
    if (isCorrect) {
      ...
      if (current.id === 'hypothesis') { setStars(1); console.log('⭐ First badge (1/3)'); }
      else if (current.id === 'conclusion') { setStars(2); console.log('⭐ Second badge (2/3)'); }
      else if (current.id === 'discovery') { setStars(3); console.log('⭐ Third badge (3/3)'); }
  ...
  const getCorrectFeedback = () => {
    const messages = ["🎉 FANTASTIC! You earned a research badge! 🌟", "🌟 AMAZING! You got it right!", "✨ WONDERFUL! Science champion!", "🎯 PERFECT! Great job!", "💫 BRILLIANT! Badge earned!"];
      ...
      <Card className={cn("w-full max-w-5xl sm:max-w-6xl lg:max-w-7xl h-[95vh] rounded-2xl sm:rounded-3xl overflow-hidden transition-all duration-500", "bg-gradient-to-br", current.bgColor, "flex flex-col")}>
        ...
        <CardContent className="p-2 sm:p-4 md:p-6 lg:p-8 flex-1 flex flex-col overflow-hidden">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-3 sm:mb-3 gap-2 sm:gap-2 flex-shrink-0">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="relative">
                <FlaskConical className="w-6 h-6 sm:w-8 sm:h-8 text-cyan-600 animate-bounce" />
                <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 text-cyan-400 absolute -top-1 -right-1 animate-ping" />
              </div>
              <div>
                <h2 className="text-base sm:text-xl font-bold text-gray-800 dark:text-white">Scientific Discovery</h2>
          ...
          {ttsInitialized && ttsAvailable && (
            <div className="mb-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-4 py-2.5 rounded-lg shadow-lg animate-fade-in">
              <div className="flex items-center gap-2 justify-center"><Volume2 className="w-4 h-4" /><span className="text-xs sm:text-sm font-bold">Quest's voice is ready! Listen carefully!</span></div>
            </div>
          )}
          ...
          <Progress value={progress} className="h-2 sm:h-2 mb-3 sm:mb-4 bg-white/30"><div className="h-full bg-gradient-to-r from-cyan-400 to-blue-400 rounded-full transition-all duration-500" /></Progress>
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden sm:overflow-hidden pb-2 sm:pb-4 scrollbar-thin">
            ...
            <div className="sm:hidden text-center h-full flex flex-col justify-center">
              <div className="relative mb-2 sm:mb-2 md:mb-3">
                ...
                <div className="flex items-center justify-center gap-1.5 sm:gap-2 mb-2 sm:mb-2">
                  {Array.from({ length: 3 }).map((_, i) => (
                    <Sparkles key={i} className={cn("w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 lg:w-8 lg:h-8 transition-all duration-500 transform hover:scale-125", i < stars ? 'text-cyan-400 animate-pulse drop-shadow-lg' : 'text-gray-300 opacity-50')} />
                  ))}
                </div>
                <div className="absolute top-1 right-1 sm:top-2 sm:right-2 animate-float-slow">
                  <Microscope className="w-6 h-6 sm:w-6 sm:h-6 md:w-8 md:h-8 text-cyan-600 opacity-70" />
                </div>
              </div>
                ...
                <>
                  <div className="mb-4 text-center">
                    <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-2">
                      ...
                      <Button variant="ghost" size="sm" onClick={playRevealText} className="text-cyan-600 hover:text-cyan-700 h-8 w-8 p-0 rounded-full" title="Replay this text with Quest voice">
                  ...
                  <div className="flex justify-center mt-4">
                    <Button onClick={handleNext} disabled={!canContinue} className={cn("bg-cyan-500 hover:bg-cyan-600 text-white rounded-xl px-8 py-3 font-bold", !canContinue && "opacity-60 cursor-not-allowed") }>
              ...
              {(current as any).listeningFirst && listeningPhase === 'listening' && (
                <div className="space-y-4 max-w-3xl mx-auto w-full">
                  <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl p-6 border-2 border-cyan-300 shadow-lg">
                    <Volume2 className="w-8 h-8 text-cyan-600 mx-auto mb-2 animate-pulse" />
                    ...
                    <Button onClick={handleReplayAudio} disabled={isPlaying} className={cn("w-full mt-3", "bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl px-6 py-3", isPlaying && "animate-pulse")}><Volume2 className="mr-2 h-4 w-4" />{isPlaying ? 'Playing...' : '🔊 Play Audio'}</Button>
              ...
              {(current as any).listeningFirst && listeningPhase === 'question' && (
                <div className="space-y-2 max-w-4xl mx-auto w-full">
                  <div className="bg-cyan-50 dark:bg-cyan-900/20 rounded-xl p-3 border border-cyan-200 dark:border-cyan-700">
                    <h4 className="text-sm sm:text-base font-bold text-gray-800 dark:text-white mb-1">{(current as any).question}</h4>
                    {showHint ? <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300">💡 {(current as any).hint}</p> : <Button variant="outline" size="sm" onClick={() => setShowHint(true)} className="text-cyan-600 border-cyan-300 hover:bg-cyan-100 text-xs">Need a hint? 🧩</Button>}
                  </div>
                  <div className="flex justify-center mb-2"><Button onClick={handleReplayAudio} disabled={isPlaying} className="rounded-xl px-5 py-2 bg-cyan-500 hover:bg-cyan-600 text-white font-bold"><Volume2 className="w-4 h-4 mr-2" />Listen Again</Button></div>
                ...
                <div className="space-y-4 max-w-4xl mx-auto w-full">
                  <div className="bg-green-100/80 dark:bg-green-900/40 rounded-2xl p-6 border-2 border-green-300 shadow-xl">
                    <h3 className="text-lg font-bold mb-3 text-gray-800 dark:text-white flex items-center justify-center gap-2">
                      {current.title}
                      <Button variant="ghost" size="sm" onClick={playRevealText} className="text-cyan-600 hover:text-cyan-700 h-7 w-7 p-0" title="Replay this text with Quest voice">
                        <Volume2 className="w-4 h-4" />
                      </Button>
                    </h3>
                    <p className="text-sm text-gray-700 dark:text-gray-200 leading-relaxed">{(current as any).revealText}</p>
                    <div className="mt-4 flex justify-center"><Button onClick={handleNext} disabled={!canContinue} className={cn("bg-cyan-500 hover:bg-cyan-600 text-white rounded-xl px-8 py-3", !canContinue && "opacity-60 cursor-not-allowed")}>Continue Adventure! 🚀</Button></div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="hidden sm:flex sm:flex-row h-full gap-4 lg:gap-6">
              <div className="sm:flex sm:flex-col sm:items-center sm:justify-center sm:w-1/4 lg:w-1/3 sm:pr-2 lg:pr-4">
                <div className="relative">
                  ...
                  <div className="flex items-center justify-center gap-2 md:gap-3 mb-4">
                    {Array.from({ length: 3 }).map((_, i) => (
                      <Sparkles key={i} className={cn("w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 transition-all duration-500 transform hover:scale-125", i < stars ? 'text-cyan-400 animate-pulse drop-shadow-lg' : 'text-gray-300 opacity-50')} />
                    ))}
                  </div>
                  <div className="absolute -top-6 -right-6 lg:-top-8 lg:-right-8 animate-float-slow">
                    <Microscope className="w-10 h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 text-cyan-600 opacity-70" />
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col justify-center sm:pr-4 overflow-hidden max-h-full">
                {!current.listeningFirst && (
                  <div className="w-full">
                    ...
                    <div className="flex justify-center">
                      <Button onClick={handleNext} disabled={!canContinue} className={cn("rounded-xl px-5 md:px-6 py-2 md:py-2.5 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-bold transition-all duration-300 hover:scale-105 shadow-lg text-sm md:text-base", !canContinue && "opacity-60 cursor-not-allowed") }>
                ...
                {(current as any).listeningFirst && listeningPhase === 'listening' && (
                  <div className="w-full">
                    <div className="bg-cyan-50/70 dark:bg-cyan-900/30 rounded-xl p-4 lg:p-5 backdrop-blur-sm border-2 border-cyan-300 shadow-xl">
                      <h3 className="text-base md:text-lg font-bold mb-3 text-gray-800 dark:text-white flex items-center justify-center gap-2">
                        <FlaskConical className="w-5 h-5 md:w-6 md:h-6 text-cyan-600 animate-bounce" />
                        <span>{(current as any).audioInstruction}</span>
                      </h3>
                      {showTranscript && accessibilityMode && (current as any).audioText && (
                        <div className="mb-3 bg-cyan-50/90 dark:bg-cyan-900/30 rounded-lg p-3 border-2 border-cyan-300">
                          <div className="flex items-center gap-2 mb-1">
                            <FileText className="w-4 h-4 text-cyan-600" />
                            <span className="text-xs font-semibold text-cyan-700 dark:text-cyan-300">Accessibility Transcript:</span>
                          </div>
                      ...
                      <div className="flex items-center justify-center gap-2 mb-4">
                        {[...Array(5)].map((_, i) => (
                          <div key={i} className="w-2 bg-cyan-500 rounded-full animate-waveform" style={{ height: '40px', animationDelay: `${i * 0.1}s` }} />
                        ))}
                      </div>
                      <div className="flex flex-col items-center gap-2 mt-3">
                        <Button onClick={handleReplayAudio} disabled={isPlaying} className={cn("rounded-xl px-5 md:px-6 py-2 md:py-2.5 text-sm md:text-base bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-bold transition-all", isPlaying && "animate-pulse")}>
                ...
                {(current as any).listeningFirst && listeningPhase === 'question' && (
                  <div className="space-y-2">
                    <div className="bg-cyan-50 dark:bg-cyan-900/20 rounded-lg p-3 border-2 border-cyan-300">
                      <h4 className="text-sm md:text-base font-bold text-gray-800 dark:text-white mb-1">{(current as any).question}</h4>
                      {showHint ? <p className="text-xs md:text-sm text-gray-600 dark:text-gray-300">💡 {(current as any).hint}</p> : <Button variant="outline" size="sm" onClick={() => setShowHint(true)} className="text-cyan-600 border-cyan-300 hover:bg-cyan-100 h-7 px-3">Need a hint? 🧩</Button>}
                    </div>
                    <div className="flex justify-center"><Button onClick={handleReplayAudio} disabled={isPlaying} className="bg-cyan-500 hover:bg-cyan-600 text-white rounded-lg px-4 py-1.5 text-xs"><Volume2 className="w-3 h-3 mr-1.5" />Replay</Button></div>
                    ...
                    {showFeedback && <div className="mt-1.5"><div className={cn("rounded-lg p-2", selectedChoice === (current as any).audioText ? "bg-green-50 dark:bg-green-900/20 text-green-600" : "bg-orange-50 dark:bg-orange-900/20 text-orange-700")}><p className="text-xs md:text-sm font-bold">{selectedChoice === (current as any).audioText ? getCorrectFeedback() : getWrongFeedback(attemptCount)}</p></div>{selectedChoice !== (current as any).audioText && retryMode && <div className="flex justify-center gap-2 mt-2"><Button onClick={handleRetry} className="bg-cyan-500 hover:bg-cyan-600 text-white rounded-lg px-5 py-2"><RotateCcw className="w-4 h-4 mr-1.5" />Try Again</Button><Button onClick={handleNext} variant="outline" className="rounded-lg px-5 py-2 border-2 border-gray-600">Skip</Button></div>}</div>}
                  </div>
                )}
                ...
                {(current as any).listeningFirst && listeningPhase === 'reveal' && (
                  <div className="bg-green-100/80 dark:bg-green-900/40 rounded-xl p-5 border-2 border-green-300 shadow-xl">
                    ...
                    <div className="mt-3 flex justify-center"><Button onClick={handleNext} disabled={!canContinue} className={cn("bg-cyan-500 hover:bg-cyan-600 text-white rounded-xl px-5 md:px-6 py-2 md:py-2.5 text-sm md:text-base", !canContinue && "opacity-60 cursor-not-allowed")}>Continue Adventure! 🚀</Button></div>
                  </div>
                )}
              </div>
            </div>
...
export default ScientificDiscoveryAdventure;
