...
import { Sparkles, Target, Star, Volume2, Play, Heart, Zap, Search, Eye, Brain, X, Ear, Award, Gauge, RotateCcw, FileText } from 'lucide-react';
...
// Detective Alex's unique voice profile (uses Google US English Female voice)
const DETECTIVE_VOICE = STORY_VOICES.Detective;

const storySteps = [
  {
    id: 'intro',
    title: '🕵️ Welcome to Mystery Detective!',
    text: 'Greetings, young investigator! I am Detective Alex, and I\'m thrilled to have you on my team!... Today, we\'re solving the Case of the Missing Masterpiece - a challenging mystery that requires sharp observation and careful listening!... You will examine clues, analyze evidence, and track down the truth... Your mission is to listen carefully to witness statements and collect THREE evidence badges!... Are you ready to begin your investigation? Let\'s solve this mystery together!',
    emoji: '🕵️',
  ...
  },
  {
    ...
    emoji: '🔍',
    character: 'Detective Alex',
    bgColor: 'from-slate-200 to-gray-200 dark:from-slate-800 dark:to-gray-800',
    interactive: true,
    listeningFirst: true,
    
    audioText: 'Evidence must be carefully preserved',
    audioInstruction: 'Listen to the investigation protocol!',
    
    question: 'What must we do with evidence?',
    hint: 'Think about how detectives protect evidence',
    
    choices: [
      { text: 'Evidence can be thrown away', emoji: '🔍🗑️', meaning: 'incorrect disposal' },
      { text: 'Evidence must be carefully preserved', emoji: '🔍✅', meaning: 'proper protection' },
      { text: 'Evidence should be ignored', emoji: '🔍👀', meaning: 'not important' }
    ],
    
    revealText: 'Excellent detective work! At every crime scene, we must carefully preserve evidence. Everything matters - a single fiber, a fingerprint, or a witness statement can solve the entire case. Remember, young investigator: details are the key to solving mysteries!',
  ...
  },
  {
    ...
    audioText: 'Forensic science reveals hidden truth',
    audioInstruction: 'Listen to the principle of forensic investigation!',
    
    question: 'What does forensic science reveal?',
    hint: 'Think about what science uncovers',
    
    choices: [
      { text: 'Forensic science reveals hidden truth', emoji: '🔬✅', meaning: 'uncovers secrets' },
      { text: 'Forensic science creates confusion', emoji: '🔬❌', meaning: 'makes things unclear' },
  ...
  {
    id: 'suspect_interview',
    title: '🎭 Suspect Questioning',
    emoji: '🎭',
    character: 'Detective Alex',
    ...
    audioText: 'Body language can reveal deception',
    audioInstruction: 'Listen to this investigative technique!',
    
    question: 'True or False: Body language can reveal deception?',
  ...
  },
  {
    ...
    audioText: 'The painting disappeared at midnight',
    audioInstruction: 'Listen to the witness\'s statement!',
    
    question: 'When did the painting disappear?',
    hint: 'Think about the time mentioned',
    
    choices: [
      { text: 'The painting disappeared at midnight', emoji: '🕛🖼️', meaning: 'nighttime disappearance' },
      { text: 'The painting disappeared in the morning', emoji: '🌅🖼️', meaning: 'daytime disappearance' },
      { text: 'The painting disappeared in the afternoon', emoji: '🌆🖼️', meaning: 'evening disappearance' }
    ],
    
    revealText: 'Great listening! The witness stated that the painting disappeared at midnight. This is a crucial piece of information! Midnight is a very specific time. Now we need to find out who had access to the gallery at that exact time. Good detectives always note the timing of events!',
  ...
  },
  {
    ...
    audioText: 'Deductive reasoning connects the clues',
    audioInstruction: 'Listen to how detectives think!',
    
    question: 'What connects the clues together?',
    hint: 'Think about the mental process of solving mysteries',
    
    choices: [
      { text: 'Random guessing connects the clues', emoji: '🧠🎲', meaning: 'luck-based' },
      { text: 'Deductive reasoning connects the clues', emoji: '🧠✅', meaning: 'logical thinking' },
  ...
  },
  {
    ...
    audioText: 'The motive becomes clear with this evidence',
    audioInstruction: 'Listen to what the breakthrough reveals!',
    
    question: 'What becomes clear with the breakthrough evidence?',
    hint: 'Think about what drives someone to commit a crime',
    
    choices: [
      { text: 'The time of day is clear', emoji: '💡🕐', meaning: 'temporal detail' },
      { text: 'The motive becomes clear with this evidence', emoji: '💡✅', meaning: 'underlying reason' },
      { text: 'Nothing important is clear', emoji: '💡😕', meaning: 'no clarity' }
    ],
    
    revealText: 'Brilliant detective work! When the evidence comes together, the motive becomes clear. Understanding Why someone did something is just as important as knowing who did it. Great detectives piece together not just what happened, but why it happened. This is your breakthrough moment!',
  ...
  },
  {
    ...
    audioText: 'Justice requires proof and careful investigation',
    audioInstruction: 'Listen to the final principle of detection!',
    
    question: 'What does justice require according to Detective Alex?',
    hint: 'Think about what makes justice fair and reliable',
    
    choices: [
      { text: 'Justice requires guessing and assumptions', emoji: '✅🎲', meaning: 'random conclusions' },
      { text: 'Justice requires proof and careful investigation', emoji: '✅🔍', meaning: 'solid evidence' },
  ...
  {
    id: 'final_reflection',
    title: '🌟 Detective Reflection',
    text: 'Outstanding work, investigator! You\'ve demonstrated excellent listening skills and sharp analytical thinking throughout this case. Every great detective knows that careful observation and logical reasoning are the keys to solving any mystery. You\'ve earned your detective\'s badge!',
    emoji: '🌟',
    character: 'Detective Alex',
...
const MysteryDetectiveAdventure = ({ onClose, onComplete }: Props) => {
  const { user } = useAuth();
  ...
  const playAudioWithCaptions = async (text: string) => {
    try {
      ...
      console.log(`🎤 Playing audio with Detective's voice:`, {
        text: cleanText.substring(0, 50) + '...',
        voice: DETECTIVE_VOICE,
        step: current.id,
  ...
  const playRevealText = async () => {
    ...
    console.log('🎤 Starting reveal text playback with Detective Alex voice:', {
    ...
    try {
      ...
      // Use playAudioWithCaptions which ensures Detective Alex's voice
  ...
  const handleSpeedChange = (newSpeed: 'normal' | 'slow' | 'slower') => {
    ...
    const playWithNewSpeed = async () => {
      try {
        ...
        await OnlineTTS.speak(cleanText, DETECTIVE_VOICE, {
          speed: newSpeed,
          showCaptions: false,
          onCaptionUpdate: () => {}
        });
  ...
  const getCorrectFeedback = () => {
    const messages = [
      "🎉 EXCELLENT! Evidence badge earned! 🌟",
      "🌟 OUTSTANDING! Correct investigation!",
      "✨ BRILLIANT! Detective work on point!",
      "🎯 PERFECT! Logic is sound!",
  ...
  const getWrongFeedback = (attempt: number) => {
    if (attempt === 1) {
      return `💪 Not quite! Listen carefully and analyze again! 🎧`;
    } else {
      return `🌟 Keep investigating! Review the evidence one more time! 👂`;
    }
  };

  const getCharacterAnimation = () => {
    if (current.id.includes('star') || current.id.includes('badge')) return 'animate-bounce';
    return 'animate-float';
  };

  const getEnvironmentIcon = () => {
    switch (current.id) {
      case 'crime_scene': return Search;
      case 'witness_statement': return Ear;
      case 'fingerprint_analysis': return Eye;
      case 'logical_deduction': return Brain;
      ...
      <Card className={cn(
        ...
        <CardContent className="p-2 sm:p-4 md:p-6 lg:p-8 flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-3 sm:mb-3 gap-2 sm:gap-2 flex-shrink-0">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="relative">
                <Target className="w-6 h-6 sm:w-8 sm:h-8 text-gray-800 dark:text-white animate-bounce" />
                <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400 absolute -top-1 -right-1 animate-ping" />
              </div>
              <div>
                <h2 className="text-base sm:text-xl font-bold text-gray-800 dark:text-white">Detective Alex's Mystery</h2>
            ...
            <div className="flex items-center gap-2 sm:gap-3 self-end sm:self-auto flex-wrap">
              ...
              <div className="flex gap-1">
                ...
                <Button
                  ...
                  className="h-7 px-2 rounded-full text-xs font-semibold shadow-sm transition-all duration-200 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-800 dark:text-gray-100"
                  title={`Playback speed (works offline & online): ${playbackSpeed === 'normal' ? 'Normal' : playbackSpeed === 'slow' ? 'Slow (Default)' : 'Very Slow'}`}
                >
                  <Gauge className="w-3.5 h-3.5 mr-1 text-gray-600 dark:text-gray-200" />
          ...
          {ttsInitialized && ttsAvailable && (
            <div className="mb-2 bg-gradient-to-r from-gray-600 to-gray-800 text-white px-4 py-2.5 rounded-lg shadow-lg animate-fade-in">
              <div className="flex items-center gap-2 justify-center">
                    <Volume2 className="w-4 h-4" />
                    <span className="text-xs sm:text-sm font-bold">
                      Detective Alex's voice is ready! Listen carefully to the evidence!
                    </span>
              </div>
            </div>
          )}
          ...
          <Progress value={progress} className="h-2 sm:h-2 mb-3 sm:mb-4 bg-white/30 flex-shrink-0">
            <div className="h-full bg-gradient-to-r from-gray-500 to-gray-700 rounded-full transition-all duration-500" />
          </Progress>
            ...
            <div className="sm:hidden text-center h-full flex flex-col justify-center">
              ...
              <div className="relative mb-2 sm:mb-2 md:mb-3">
                ...
                <div className="absolute top-1 right-1 sm:top-2 sm:right-2 animate-float-slow">
                  <EnvironmentIcon className="w-6 h-6 sm:w-6 sm:h-6 md:w-8 md:h-8 text-gray-600 opacity-70" />
                </div>
              </div>
                      ...
                      >
                        {isPlaying ? (
                          ...
                          <>
                            ...
                            <span className="hidden sm:inline">Listen to the Evidence</span>
                  ...
                  <div className="bg-green-100/80 dark:bg-green-900/40 rounded-2xl p-6 backdrop-blur-sm border-2 border-green-300 shadow-2xl">
                    <h3 className="text-lg sm:text-xl font-bold mb-3 text-gray-800 dark:text-white flex items-center justify-center gap-2 sm:gap-2">
                      ...
                      <Button 
                        ...
                        title="Replay this text with Detective Alex's voice"
                      >
                    ...
                    <div className="mt-4 flex justify-center">
                      <Button 
                        onClick={handleNext} 
                        className="bg-gradient-to-r from-gray-500 to-slate-500 hover:from-gray-600 hover:to-slate-600 text-white font-bold rounded-2xl px-8 py-3"
                      >
              ...
              <div className="sm:flex sm:flex-col sm:items-center sm:justify-center sm:w-1/4 lg:w-1/3 sm:pr-2 lg:pr-4">
                <div className="relative">
                  ...
                  <div className="absolute -top-6 -right-6 lg:-top-8 lg:-right-8 animate-float-slow">
                    <EnvironmentIcon className="w-10 h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 text-gray-600 opacity-70" />
          </div>
          </div>
              </div>
                ...
                <div className="w-full">
                  <div className="bg-green-100/80 dark:bg-green-900/40 rounded-xl p-4 lg:p-5 backdrop-blur-sm border-2 border-green-300 shadow-xl">
                    ...
                    <div className="mt-3 flex justify-center">
                      <Button 
                        onClick={handleNext} 
                        className="bg-gradient-to-r from-gray-500 to-slate-500 hover:from-gray-600 hover:to-slate-600 text-white font-bold rounded-xl px-5 md:px-6 py-2 md:py-2.5 text-sm md:text-base"
                      >
                        Continue Investigation! 🚀
                      </Button>
                    </div>
              ...
              {!current.listeningFirst && (
                <div className="w-full">
                  ...
                  <div className="flex justify-center">
                    <Button 
                      onClick={handleNext} 
                      className="rounded-xl px-5 md:px-6 py-2 md:py-2.5 bg-gradient-to-r from-gray-500 to-slate-500 hover:from-gray-600 hover:to-slate-600 text-white font-bold transition-all duration-300 hover:scale-105 shadow-lg text-sm md:text-base"
                    >
                      {stepIndex === storySteps.length - 1 ? (
                        <>
...
export default MysteryDetectiveAdventure;