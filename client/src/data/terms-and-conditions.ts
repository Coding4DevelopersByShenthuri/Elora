export const TERMS_AND_CONDITIONS = `
Terms and Conditions Agreement

www.speakbee.com 
elora.toinfo@gmail.com
Last Updated: ${new Date().toLocaleDateString()}

The Agreement

This Service Agreement ("Contract") is made and entered into as of the date of your acceptance, by and between Elora ("Service Provider") and You ("User" or "Client"). By accessing, registering for, or using the services of Elora, you acknowledge that you have read, understood, and agree to be bound by the terms and conditions outlined in this Agreement.

Introduction

Welcome to Elora! By using our application and/or using the services that are provided, you acknowledge that you have read, understood, and agree to be bound by our Terms and Conditions. These Terms and Conditions unconditionally extend and apply to all related applications, services, and updates. If you do not agree with all of these Terms and Conditions, you are prohibited from using this Application, and you may discontinue use immediately. Elora recommends that you save or print a copy of these Terms and Conditions for future reference.

Definitions/Terminology

The following definitions apply to these Terms and Conditions, Privacy Policy, and all related Agreements:

- "User", "Visitor," "Client," "Customer," "You" and "Your" refers to you, the person(s) that use this Application.
- "Elora", "We", "Our" and "Us" refers to our Application/Company.
- "Party," "Parties," or "Us" refers to both you and us.
- "Application" refers to the Elora Spoken English Training software and all related services.
- "Content" refers to all materials, information, documents, services, or other entities available through the Application.

All terms refer to all considerations of Elora necessary to undertake support to you for the express purpose of meeting your User needs in respect of our services, under and subject to, prevailing law of the jurisdiction in which Elora operates.

User Obligations

Accurate Information
You must provide accurate, current, and complete information during the registration process and maintain the accuracy of such information.

Compliance with Laws
You must comply with all applicable laws, regulations, and third-party agreements in your use of the Application.

Lawful Use
You will use Elora's services only for lawful purposes and as outlined in this Agreement. Any unauthorized or prohibited use may result in immediate termination of your account.

Account Security
You are responsible for maintaining the confidentiality of your login credentials and for all activities that occur under your account.

Product/Service Details

Elora offers the following services through its offline-first English language learning application:

- Speech Recognition & Pronunciation Feedback: Real-time speech analysis and corrective feedback
- Interactive Learning Modules: Structured lessons for different proficiency levels
- Conversation Practice: AI-powered conversational practice sessions
- Progress Tracking: Comprehensive learning progress monitoring
- Gamified Learning: Engaging educational games and achievements
- Category-Specific Training: 
  - Kids module (ages 6-12)
  - Adult learners (Beginner to Advanced)
  - IELTS/PTE exam preparation
- Offline Functionality: Core features available without internet connectivity

Intellectual Property Rights

Our Intellectual Property
Other than the content you own and opt to include on this Application, under these Terms, Elora and/or its licensors own and reserve all intellectual property rights of this Application. You are granted a limited license, subject to the restrictions entailed in these Terms and Conditions, for purposes of using this Application's content and services.

Protected intellectual property includes:
- AI models, algorithms, and machine learning systems
- Lesson content, curriculum, and educational materials
- User interface design and application architecture
- Branding, trademarks, and proprietary technology
- Speech recognition and processing systems

Your Content
You retain ownership of:
- Your personal voice recordings created during practice sessions
- Your personal learning data and progress information
- User-generated content where applicable

License Grant
By using the Application, you grant Elora a worldwide, non-exclusive, royalty-free license to use, reproduce, and process your data solely for the purpose of providing and improving our services, in accordance with our Privacy Policy.

Service Specifications

Technical Requirements
The Application requires:
- Mobile Devices: Android 9.0+ or iOS 13.0+
- Desktop Systems: Windows 10+ or macOS Catalina 10.15+
- Storage: Minimum 1GB available storage space
- Hardware: Functional microphone for speech recognition features
- Memory: Minimum 2GB RAM for optimal performance

Offline Capabilities
- Core learning features operate without internet connectivity
- Voice processing occurs locally on your device
- Progress data stored locally with optional cloud synchronization
- Periodic updates required for new content and features

Payment and Financial Terms

Service Models
Elora may utilize the following revenue models:
- One-time application purchase
- Subscription-based premium features
- Institutional bulk licensing
- Certification and examination fees
- In-app purchases for additional content

Payment Terms
- All payments are final and non-refundable except as required by law
- Subscription services auto-renew unless cancelled before renewal date
- Pricing may change with 30 days notice to current subscribers
- You are responsible for any taxes applicable in your jurisdiction

Free Trial Offers
Where offered, free trials automatically convert to paid subscriptions unless cancelled before the trial period ends.

User Responsibilities and Conduct

Permitted Uses
You may use the Application for:
- Personal English language learning and improvement
- Educational purposes in accredited institutions
- Professional development and career advancement
- Examination preparation and practice
- Non-commercial personal use

Prohibited Activities
You may not:
- Reverse engineer, decompile, or disassemble the Application
- Bypass or attempt to bypass any security features
- Use the Application for any illegal or unauthorized purpose
- Share account access or create multiple fraudulent accounts
- Use the Application to harass, abuse, or harm others
- Distribute viruses or malicious code through our services
- Violate intellectual property rights of Elora or third parties

Privacy and Data Protection

Data Processing Architecture
- Voice data and user interactions processed locally on your device
- Personal voice data not transmitted to external servers for core functionality
- Optional cloud services for backup and synchronization
- Anonymous usage data collected for service improvement

Data Collection
We collect and process:
- Account registration information
- Learning progress and performance data
- Application usage statistics and analytics
- Device information for compatibility purposes
- Optional voice data for personalized learning (with consent)

Data Security
- Local data storage with encryption capabilities
- Secure authentication and password protection
- Regular security updates and vulnerability patches
- Compliance with applicable data protection regulations

Disclaimer of Warranties

THE APPLICATION AND SERVICES ARE PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED. TO THE FULLEST EXTENT PERMISSIBLE PURSUANT TO APPLICABLE LAW, Elora DISCLAIMS ALL WARRANTIES, EXPRESS OR IMPLIED, INCLUDING, BUT NOT LIMITED TO:

- Perfect speech recognition accuracy or performance
- Uninterrupted or error-free service availability
- Specific learning outcomes or examination results
- Compatibility with all devices, configurations, or networks
- Timely delivery of updates or new features
- Effectiveness for individual learning styles or needs

Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY APPLICABLE LAW, IN NO EVENT SHALL Elora, ITS DIRECTORS, EMPLOYEES, PARTNERS, AGENTS, SUPPLIERS, OR AFFILIATES BE LIABLE FOR:

- Technical issues related to user device hardware or software
- Internet connectivity requirements for certain features
- User's ability to achieve specific learning goals or outcomes
- Third-party services, applications, or integrations
- Indirect, incidental, special, consequential, or punitive damages
- Any damages exceeding the amount paid by you to Elora in the past six months

Modifications to Terms

We reserve the right to:
- Update or modify these Terms at any time
- Notify users of significant changes via email or in-app notifications
- Require acceptance of updated Terms for continued service use
- Implement changes immediately for legal or security reasons

Continued use of the Application after changes constitutes acceptance of modified Terms.

Termination and Suspension

By You
You may terminate your account at any time through application settings or by contacting customer support.

By Elora
We may suspend or terminate accounts for:
- Violation of these Terms and Conditions
- Prolonged account inactivity (typically 12 months)
- Security concerns or fraudulent activities
- Legal or regulatory requirements
- Non-payment of applicable fees

Effect of Termination
Upon termination:
- Your right to use the Application immediately ceases
- Access to your account and learning data may be revoked
- Provisions regarding intellectual property, warranties, and liability limitations survive termination

Special User Categories

For Parents/Guardians (Kids Module)
- You are responsible for supervising children's use of the Application
- Ensure age-appropriate content and reasonable usage limits
- Monitor learning progress and engagement
- Contact us immediately with any content suitability concerns

For Educational Institutions
- Bulk licensing requires separate institutional agreements
- Responsible for user management and compliance monitoring
- Must maintain appropriate acceptable use policies
- Additional terms may apply for classroom implementations

For Examination Candidates
- Mock test scores are for practice purposes only
- Not a guarantee of actual examination performance or results
- Independent verification required for official certification
- Examination content may not reflect actual test materials

Governing Law and Dispute Resolution

Governing Law
These Terms shall be governed by and construed in accordance with the laws of India, without regard to its conflict of law provisions.

Dispute Resolution
Any disputes arising from these Terms or your use of the Application shall be resolved through:
1. Informal negotiation between parties
2. Mediation where appropriate
3. Binding arbitration in accordance with Indian arbitration laws
4. Legal proceedings in courts of competent jurisdiction in Mumbai, India

Class Action Waiver
You agree to resolve disputes on an individual basis and waive any right to participate in class actions or representative proceedings.

Contact Information and Support

For questions, concerns, or support regarding these Terms and Conditions, contact:

Email: elora.toinfo@gmail.com 
Website: www.speakbee.com  
Support Hours: Monday-Friday, 9:00 AM - 6:00 PM IST

We typically respond to inquiries within 2 business days.

Entire Agreement

These Terms and Conditions, together with our Privacy Policy and any additional agreements you may enter with Elora, constitute the entire agreement between you and Elora regarding the Application and supersede all prior agreements, understandings, and representations.

Severability

If any provision of these Terms is found to be unenforceable or invalid, that provision shall be limited or eliminated to the minimum extent necessary so that these Terms shall otherwise remain in full force and effect and enforceable.

No Waiver

Our failure to enforce any right or provision of these Terms will not be considered a waiver of those rights. The waiver of any such right or provision will be effective only if in writing and signed by our duly authorized representative.

Assignment

You may not assign or transfer these Terms, by operation of law or otherwise, without Elora's prior written consent. Any attempt by you to assign or transfer these Terms without such consent will be null and of no effect. Elora may assign or transfer these Terms, at its sole discretion, without restriction.

By using Elora, you acknowledge that you have read, understood, and expressly agree to be bound by these Terms and Conditions.
`;

// Security questions for user registration
export const SECURITY_QUESTIONS = [
  "What is your mother's maiden name?",
  "What city were you born in?",
  "What is the name of your first pet?",
  "What was your favorite school teacher's name?",
  "What is your favorite movie?",
  "What is your favorite book?",
  "What is the name of the street you grew up on?",
  "What is your favorite food?",
  "What is your dream job?",
  "What is your favorite vacation spot?"
];